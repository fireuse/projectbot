...
public class CreateHandler implements Consumer<ChatInputInteractionEvent> {
    ...
    public void accept(ChatInputInteractionEvent event) {
        try {
            ...
            createChannels(g, description, projectRole, team);
            ProjectManager.addProject(new Project(name, event.getInteraction().getMember().get().getId(), description, projectRole.getId(), team));
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }

    private static void createChannels(Guild g, String description, Role role, Snowflake team) {
